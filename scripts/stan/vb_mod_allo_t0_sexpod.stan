...
model {
 ...
 for(i in 1:n_obs_k){
    if(type[i] == 1){ // both
      obs_mean[i,1] = par_k[id[i],1]*(1-inv_logit(par_k[id[i],2])^(age[i] + t0p));
      obs_mean[i,2] = par_k[id[i],3]*(1-inv_logit(par_k[id[i],4])^(age[i] + t0p));
      target += multi_normal_cholesky_lpdf(val[i]| obs_mean[i], Lobs);
    } else if(type[i] == 2) { // z
      target += normal_lpdf(val[i,2]| par_k[id[i],3]*(1-inv_logit(par_k[id[i],4])^(age[i] + t0p)), sigma_obs[2]);
    } else if(type[i] == 3) { // y
      target += normal_lpdf(val[i,1]| par_k[id[i],1]*(1-inv_logit(par_k[id[i],2])^(age[i] + t0p)), sigma_obs[1]);
    }
 ...
 for(i in (n_obs_k+1):n_obs){
    if(type[i] == 1){ // both
      obs_mean_0[i,1] = par_u_0[id[i],1]*(1-inv_logit(par_u_0[id[i],2])^(age[i] + t0p));
      obs_mean_0[i,2] = par_u_0[id[i],3]*(1-inv_logit(par_u_0[id[i],4])^(age[i] + t0p));
      obs_mean_1[i,1] = par_u_1[id[i],1]*(1-inv_logit(par_u_1[id[i],2])^(age[i] + t0p));
      obs_mean_1[i,2] = par_u_1[id[i],3]*(1-inv_logit(par_u_1[id[i],4])^(age[i] + t0p));
      ...
      tmp[id[i],1] += normal_lpdf(val[i,2]| par_u_0[id[i],3]*(1-inv_logit(par_u_0[id[i],4])^(age[i] + t0p)), sigma_obs[2]);
      tmp[id[i],2] += normal_lpdf(val[i,2]| par_u_1[id[i],3]*(1-inv_logit(par_u_1[id[i],4])^(age[i] + t0p)), sigma_obs[2]);
    } else if(type[i] == 3) { // y
      tmp[id[i],1] += normal_lpdf(val[i,1]| par_u_0[id[i],1]*(1-inv_logit(par_u_0[id[i],2])^(age[i] + t0p)), sigma_obs[1]);
      tmp[id[i],2] += normal_lpdf(val[i,1]| par_u_1[id[i],1]*(1-inv_logit(par_u_1[id[i],2])^(age[i] + t0p)), sigma_obs[1]);
    }
 }
