...
Table S2. Model parameter estimates of the expanded model exploring sex($\beta$)- and pod($\gamma$)-effects, including the mean ($\mu$), standard deviation ($\sigma$), correlation ($\rho$), and theoretical age less than zero when the length was zero ($t_0$).
...
Fig. S5. Number of snapshots where an individual dolphin measurement of TL and/or BHDF was taken per pod (a) over the course of the entire study and (b) per trip sampling effort.
