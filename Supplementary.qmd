...
We re-ran our model with data where age was adjusted by an additional five years at each measurement occasion for 20 individuals not seen in their birth year (this excluded known reproductive females which already had an applied approach for adjusting age). These results yielded slightly higher logit($k_{hi}$) estimates (Table S1.3.1) corresponding to slower growth rates (differences between median estimates of \< 0.035 on the logit scale) (Fig. 1.3.1), which is reasonable given the added time to reach the measured lengths. It is likely that an age adjustment is appropriate for some of these individuals, but it is impossible to know with the sighting histories we have which individuals these are. As an example, the unknown aged individual first seen most recently in this dataset (Reel, a Dusky pod male) was first sighted in 2008, and we found no differences between modeled growth curves (Fig. 1.3.2). Finally, this age adjustment had no impact on overall findings related to effects from pod and sex membership as the range of median estimates for the Doubtful pod and for females encompass the values from all other groups (Fig. 1.3.3). Overall, these modeled results yield minor differences from the results presented in the main text and our substantive conclusions are unchanged.
...
::: {layout-nrow="1"}
![](./Figures/plus_age/catplot_2024-06-21.png){width="49%"} ![](./Figures/plus_age/catplot_2024-12-13_5.png){width="49%"}
:::

Fig. 1.3.1 Caterpillar plots of the estimated growth rate parameters (\[2\] = logit($k_1$), \[4\] = logit($k_2$)), including mean ('mu') and associated standard deviation ('sigma') estimates using the original data formulation in the main text (left) and data where five years were added to the age of unknown aged individuals (excluding reproductive females) at measurement occasions (right).

::: {layout-nrow="1"}
![](./Figures/indplot_76_t0.png){width="49%"} ![](Figures/plus_age/indplot_76_p5.png){width="49%"}
:::

Fig. 1.3.2 Growth curves estimated for Reel, the most recently added unknown aged individual to the catalogue in this dataset, where a) is the modeled growth curve without adjustments to age (first year sighted is used as birth year), and b) is the modeled growth curve adding five years to the assigned age at each measurement occasion.
