...
Individual random effects can introduce issues with model fit under the centered-parameterisation. We experienced this when fitting our model, therefore, we implemented a non-centered paramaterisation with individual random effects represented by $\zeta_{t_{0_i}}$ to estimate $t_{0_i}$,

\vspace{-3ex}

```{=tex}
\begin{equation}
t_{0_i} = \mathrm{exp}(\mu_{t_0} + \sigma_{t_0} \times \zeta_{t_{0_i}}),\; i = 1,\dotsc, n,
\end{equation}
```
where $\zeta_{t_{0_i}}$ has a standard normal distribution. For the multivariate growth parameter model, $\boldsymbol{\theta_{i}}$, we introduce a vector of individual random effect parameters, $\boldsymbol{\zeta_i}$ = \[$\zeta_{L_{1i}}$, $\zeta_{k_{1i}}$, $\zeta_{L_{2i}}$, $\zeta_{k_{2i}}$\], and find
...
\end{equation}
```
The vector $\boldsymbol{\zeta_i}$ was assumed to come from a multivariate normal distribution with zero mean and identity variance matrix. The above formulations are equivalent to the log-normal and normal distributions detailed in Equation 2 and 3 in the main body of the text, respectively.
...
& = 2.739 + 0.259 \times \mathrm{logit}(k_{1i}),
\end{split}
\end{equation}
```
...
\newline
Fig. S3. Example photographs of squid sucker marks on Fiordland bottlenose dolphins (pers comm. K. Bolstad) including (a) on the chin in 2015 and (b) left of the eye in 2022.
:::
...
Model code to confirm exploratory findings that sex and pod do not influence growth parameters. Results are below in Table S3.
...
Table S3. Model estimates of the mean ($\mu$) and standard deviation ($\sigma$) of the parameters specifically estimated in the expanded model to explore sex and pod effects.
