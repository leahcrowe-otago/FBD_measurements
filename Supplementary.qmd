...
priors = data.frame(Parameter = c("all $\\mu$", "all $\\sigma$", "$L$","$t_0$"), `Prior_distribution` = c("N(0, 1000)", 
                                 "t_{3}(0, 50)", "LKJ(1)","Lognormal(0, 10)"))
...
Table S3. Model parameter estimates of the expanded model (Appendix II) exploring sex($\beta$)- and pod($\gamma$)-effects, including the mean ($\mu$), standard deviation ($\sigma$), correlation ($\rho$), and theoretical age less than zero when the length was zero ($t_0$).
...
cat(paste(readLines("./scripts/stan/vb_mod_allo_t0.stan"), collapse = "\n"))
```
...
cat(paste(readLines("./scripts/stan/vb_mod_allo_t0_sexpod.stan"), collapse = "\n"))
```
