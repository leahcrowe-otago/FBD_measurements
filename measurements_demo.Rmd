...
measurements_demo_ind<-measurement_ID%>%
  ...
  dplyr::select(trip, ID, actual_length_mean, actual_length_sd, n, SEX, BIRTH_YEAR, FIRST_YEAR, ends_with(calf_year), POD)%>%
...
ggplot(age_length%>%filter(!is.na(AGECLASS)))+
  geom_boxplot(aes(x=AGECLASS, y = actual_length_mean, color = POD))+
  facet_wrap(~trip)
