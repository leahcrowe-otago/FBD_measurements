...
model<-nimble::nimbleCode({
  ...
  for(i in 1:n){
    #likelihood
    #y = expected length (L_t)
    #x = Linf
    
    # y = y[i](t[j]) eq. 1 observed length
    # x = g[i](t) = predicted length
    y[i] ~ T(dnorm(x[i], sd = sigma), 0, Inf)
    #E[i] ~ dnorm(0, sigma)
    x[i] <- Linf[i]*(1 - k^(age[i] - t0)) #+ E
    #E[i] <- (y[i] - x[i])^2  
...
samples<-nimble::nimbleMCMC(code = model,
                            constants = list(n = n),
                            data = list(y = age_vbgc$length, age = age_vbgc$age_month),
                            inits = inits_fun(),
                            niter = 1000,
                            nburnin = 200, 
                            nchains = 3,
...
ggplot()+
  geom_histogram(mapping = aes(x = as.matrix(samples), y=..density..), bins = 30, color = "black", alpha = 0.8)+
