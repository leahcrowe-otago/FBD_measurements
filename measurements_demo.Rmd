...
age_calc<-age_calc%>%
  ...
  mutate(age = case_when(
    age_value == "min" & !is.na(FIRST_CALF) & (as.numeric(FIRST_CALF) - as.integer(avg_primo_age)) < as.numeric(FIRST_YEAR) ~ age + (as.integer(avg_primo_age) - (as.numeric(FIRST_CALF) - as.numeric(FIRST_YEAR))),
    TRUE ~ age
  ))%>%
...
```{r vbgc_data_sex, include = T, warning = F, message = F}
...
ij_sex_ID<-ID_index
saveRDS(ij_sex_ID, file = paste0("ij_sex_ID.rds"))

#both
age_vbgc_sex_both<-age_calc%>%
  filter(SEX != 'X')%>%
  filter(!is.na(length_use) & !is.na(BHDF))%>% #filter for tl modeling
  dplyr::rename("length" = "length_use")  
nrow(age_vbgc_sex_both)

#only bhdf
age_vbgc_sex_bhdf<-age_calc%>%
  filter(SEX != 'X')%>%
  filter(is.na(length_use) & !is.na(BHDF))%>% 
  dplyr::rename("length" = "length_use")  
nrow(age_vbgc_sex_bhdf)

#only tl, no bhdf
age_vbgc_sex_tl<-age_calc%>%
  filter(SEX != 'X')%>%
  filter(!is.na(length_use) & is.na(BHDF))%>% 
  dplyr::rename("length" = "length_use")
nrow(age_vbgc_sex_tl)

# 1 = both, 2 = bhdf only, 3 = tl only
age_vbgc_sex<-list(age_vbgc_sex_both, age_vbgc_sex_bhdf, age_vbgc_sex_tl)
length(age_vbgc_sex)
age_vbgc_sex[[1]]

for(i in 1:length(age_vbgc_sex)){

  ij<-age_vbgc_sex[[i]]%>%
    arrange(ID, age_month)%>%
    ungroup()%>%
    left_join(ID_index, by = c("ID"))%>%
    #mutate(max_j = 1:n())%>%
    group_by(ID)%>%
    mutate(obs = 1:n(),
           #J = max(max_j),
  ...
  if (i == 1){
    ij<-ij%>%
      dplyr::select(ID, ind, obs, age, sex, length, BHDF)
  }
...
saveRDS(ij, file = paste0("ij_sex_",i,".rds"))
