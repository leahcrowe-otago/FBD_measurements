...
measurements_demo_ind<-length_data%>%
  ...
  dplyr::select(trip, YEAR, ID, length_use, BHDF, SEX, MOM, BIRTH_YEAR, INUTERO, FIRST_DATE, FIRST_YEAR, FIRST_CALF, ends_with('2022'), ends_with('2023'), ends_with('2024'), POD, month)%>%
...
raw<-ggplot(age_calc)+
  geom_point(mapping = aes(x = age, y = length_use, color = POD))+
  geom_point(mapping = aes(x = age, y = BHDF, color = POD), shape = 21)+
  theme_bw()+
  ylab("Length (m)")+
  theme(legend.position = "bottom")
...
saveRDS(ij, file = paste0("ij_",i,".rds"))

}

```
```{r vbgc, include = F, warning=FALSE, message=FALSE}
# VBGC frequ
# data for VBGC
...
age_calc%>%
  filter(age == 1 & !is.na(length_use))%>%
  group_by(age)%>%
  mutate(mean = mean(length_use))
...
\newpage
```{r vbgc_bayes, include = T, warning=FALSE, message=FALSE}
...
ggplot()+
  geom_histogram(mapping = aes(x = log(samples.df$K), y=..density..), bins = 30, color = "black", alpha = 0.8)+
  geom_density(mapping = aes(x = log(samples.df$K)), color = "black")+
  theme_bw()
...
ggplot(age_vbgc)+
  ...
  stat_function(fun=function(x)Linf_mean*(1 - K_mean^(x + t0_mean)))+
  theme(legend.position = "bottom")+
  xlab("Age")+
  ylab("Mean length (m)")+
  theme_bw()+
...
```
```{r vbgc_bayes_heir, echo = T, warning=FALSE, message=FALSE}
##individuals level heirarchical modeling

## called in measurements_demo.Rmd file
# L. Crowe Jan 2024

source('./scripts/individuals_vbgc.R', local = TRUE, verbose = F)$value
...
mean_K_Linf_t0p_ind<-mean_summ_df_rn%>%
  filter(!is.na(index))%>%
...
vb_bayes_ind<-ij%>%
  left_join(mean_K_Linf_t0p_ind, by = c("i" = "index"))%>%
  dplyr::rename("index" = "i")%>%
  left_join(lifehist, by = c("ID" = "NAME"))%>%
  dplyr::select(ID, index, ind, j, J, age_month, length, K, Linf, POD, SEX, FIRST_CALF, BIRTH_YEAR, FIRST_YEAR)
...
#for (i in 1:5){
  ...
  new_layer <- expr(
    stat_function(mapping = aes(x = data_first$age_month[!!i], y = data_first$length[!!i], color = as.factor(data_first$SEX[!!i])), fun=function(x)(data_first$Linf[!!i] * (1 - exp(-data_first$K[!!i] * (x - mean_sigma_y_t0$t0p))))) #bayes ind
  )
...
vb_Linf_K_demo<-vb_bayes_ind%>%distinct(ID, K, Linf, POD, FIRST_YEAR, BIRTH_YEAR)

# k per birth year per pod
# ggplot(vb_Linf_K_demo, aes(x = as.factor(BIRTH_YEAR), y = K, color = POD))+
#   geom_boxplot()+
#   geom_point()

#comparing theoretical max length per birth year
ggplot(vb_Linf_K_demo, aes(x = as.factor(BIRTH_YEAR), y = Linf))+
  geom_boxplot()+
  geom_point()
...
growth_curve <- Calculate_MCMC_growth_curve(fit, Model = "VB",
                                            max.age = max(age_vbgc$age_month), probs = c(.5,.95))
