...
measurements_demo_ind<-length_data%>%
  ...
  dplyr::select(trip, YEAR, ID, length_use, BHDF, SEX, BIRTH_YEAR, FIRST_YEAR, FIRST_CALF, ends_with('2022'), ends_with('2023'), ends_with('2024'), POD, month)%>%
...
ggplot(age_calc)+
  geom_point(mapping = aes(x = length_use, y = BHDF, color = POD))
...
ij<-age_vbgc[[i]]%>%
  ...
  dplyr::select(ID, i, ind, j, J, age_month, length, BHDF)%>%
  ungroup()
...
saveRDS(age_ij, file = paste0("age_ij_,",i,",.rds"))
...
length_ij_noID<-length_ij%>%
  dplyr::select(-ID)

colnames(length_ij_noID)<-NULL
saveRDS(length_ij_noID, file = paste0("length_ij_",i,".rds"))

length_ij_ID<-length_ij%>%
  dplyr::select(ID)
saveRDS(length_ij_ID, file = paste0("length_ij_ID_",i,".rds"))
