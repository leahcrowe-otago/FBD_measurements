...
measurements_demo_ind<-measurement_ID_trip%>%
  ...
  dplyr::select(trip, YEAR, ID, actual_length_mean, actual_length_sd, n, SEX, BIRTH_YEAR, FIRST_YEAR, ends_with('2022'), ends_with('2023'), ends_with('2024'), POD, month)%>%
...
## include each ID average per trip, but need to adjust for fractional age
ID_pertrip_pod<-ggplot(age_calc, aes(x = age, y = actual_length_mean, color = POD))+
  geom_point()+
...
individuals_measuredmanytimes<-ggplot(ID_age, aes(x = trip, y = actual_length_mean, color = SEX))+
  geom_point()+
  geom_errorbar(aes(ymin=actual_length_mean-actual_length_sd, ymax=actual_length_mean+actual_length_sd))+
  facet_wrap(~ID)+
  theme(legend.position = "bottom")

ggsave("./Figures/individuals_measuredmanytimes.png", individuals_measuredmanytimes, dpi = 320, height = 300, width = 500, units = 'mm')
...
## length per year age
mean_year_age_noid<-ggplot(age_calc_year_mean, aes(x = age, y = actual_length_mean_mean))+
  geom_point()+
  #geom_smooth()+
  theme(legend.position = "bottom")+
  xlab("Age")+
...
#mean length per age
# age_vbgc<-age_calc_year_mean%>%
#   ungroup()%>%
#   group_by(age)%>%
#   mutate(length = mean(actual_length_mean_mean))%>%
#   distinct(age, length)%>%
#   arrange(age)

#mean length per individual at distinct ages pooled by year, first year = 1
# age_vbgc<-age_calc_year_mean%>%
#   ungroup()%>%
#   mutate(length = actual_length_mean_mean,
#          age = age+1)%>%
#   distinct(ID, age, length)%>%
#   arrange(age)

#decimal ages
age_vbgc<-age_calc_month%>%
  ungroup()%>%
  group_by(age)%>%
  mutate(length = mean(actual_length_mean))%>%
  distinct(ID, age, length, age_value)%>%
  arrange(age)%>%
  left_join(lifehist, by = c("ID" = "NAME"))%>%
  ungroup()%>%
...
SSQ <- function(theta, x) {
    ...
    for (i in 1:nrow(age_vbgc)) {
        lpred[i] <- Linf * (1 - exp(-K * (age_vbgc$age[i] - t0)))
...
ggplot(age_vbgc_curve)+
  geom_point(mapping = aes(x = age, y = length, color = age_value))+
