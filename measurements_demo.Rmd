...
hist<-ggplot(measurement_ID_trip)+
  geom_histogram(aes(x = n), binwidth = 1, color = "black")+
  facet_wrap(~trip)+
  xlab("Number of individuals")+
  ylab("Number of photographs measured")+
  theme_bw()
...
age_length%>%ungroup()%>%distinct(ID, POD)%>%filter(!is.na(POD))%>%group_by(POD)%>%tally()
...
mean_trip_age<-ggplot(age_calc_month, aes(x = age, y = actual_length_mean))+
  geom_point()+
...
age_calc_month%>%filter(ID == 'DANGER')

ggsave("./Figures/ mean_trip_age.png", mean_trip_age, dpi = 320, height = 100, units = 'mm')
...
ageclass<-ggplot(ageclass_length_peryear)+
...
sex<-ggplot(agesex_length_peryear)+
  geom_boxplot(aes(x=AGECLASS, y=actual_length_mean_mean, color = POD, fill = POD), alpha = 0.5, varwidth = T)+
  facet_wrap(~SEX)+
  theme(legend.position = "bottom")+
  xlab("Age class")+
