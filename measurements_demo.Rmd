...
measurements_demo_ind<-measurement_ID_trip%>%
  ...
  dplyr::select(trip, YEAR, ID, actual_length, SEX, BIRTH_YEAR, FIRST_YEAR, FIRST_CALF, ends_with('2022'), ends_with('2023'), ends_with('2024'), POD, month)%>%
...
length_all<-ggplot(age_calc, aes(x = age_month, y = actual_length, shape = POD, color = age_value))+
  geom_point()+
  theme_bw()+
...
length_trip<-ggplot(age_calc, aes(x = age_month, y = actual_length, color = POD))+
  geom_point()+
  facet_wrap(~trip)+
  theme_bw()+
  theme(legend.position = "bottom")+
...
ID_age<-age_calc%>%
  ...
  filter(n > 1)%>%
  arrange(ID,trip)%>%
  mutate(length_diff = lead(actual_length) - actual_length)
  
individuals_measuredmanytimes<-ggplot(ID_age, aes(x = trip, y = actual_length, color = SEX))+
  geom_point()+
...
```{r, vbgc_bayes}
library(brms);library(nimble)

age_calc

model<-nimble::nimbleCode({
  
  for(i in 1:nrow(age_calc))
})
